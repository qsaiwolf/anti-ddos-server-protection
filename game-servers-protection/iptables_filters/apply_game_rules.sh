...
if [ "$ACTION" == "flush" ] || [ "$ACTION" == "unload" ]; then
...
if [ "$SOURCE_ENGINE_ENABLE" == "true" ]; then
    ...
    # Optimization: Use hashlimit FIRST before string matching to save CPU during massive floods
    iptables -A GAME_PROTECTION -p udp --dport "$SOURCE_ENGINE_PORT" -m hashlimit --hashlimit-above "$SOURCE_ENGINE_LIMIT/sec" --hashlimit-burst 10 --hashlimit-mode srcip --hashlimit-name source_limit -j DROP
    # String match for A2S_INFO query payload (0xFFFFFFFF54)
    iptables -A GAME_PROTECTION -p udp --dport "$SOURCE_ENGINE_PORT" -m string --algo bm --hex-string "|ff ff ff ff 54|" -j ACCEPT
fi
...
if [ "$SAMP_MTA_ENABLE" == "true" ]; then
    ...
    iptables -A GAME_PROTECTION -p udp --dport "$SAMP_MTA_PORT" -m hashlimit --hashlimit-above "$SAMP_MTA_LIMIT/sec" --hashlimit-burst 20 --hashlimit-mode srcip --hashlimit-name samp_limit -j DROP
fi

# 4. FiveM Protection (TCP/UDP)
if [ "$FIVEM_ENABLE" == "true" ]; then
    echo "🛡️ Enabling FiveM Protection on port $FIVEM_PORT..."
    iptables -A GAME_PROTECTION -p tcp --syn --dport "$FIVEM_PORT" -m connlimit --connlimit-above "$FIVEM_CONNLIMIT" --connlimit-mask 32 -j REJECT --reject-with tcp-reset
    iptables -A GAME_PROTECTION -p udp --dport "$FIVEM_PORT" -m hashlimit --hashlimit-above 50/sec --hashlimit-burst 100 --hashlimit-mode srcip --hashlimit-name fivem_udp -j DROP
...
if [ "$TS3_ENABLE" == "true" ]; then
    ...
    iptables -A GAME_PROTECTION -p udp --dport "$TS3_PORT" -m hashlimit --hashlimit-above "$TS3_UDP_LIMIT/sec" --hashlimit-burst 20 --hashlimit-mode srcip --hashlimit-name ts3_limit -j DROP
fi
...
if [ "$SEVEN_DAYS_ENABLE" == "true" ]; then
    ...
    iptables -A GAME_PROTECTION -p tcp --syn --dport "$SEVEN_DAYS_PORT" -m connlimit --connlimit-above "$SEVEN_DAYS_CONNLIMIT" --connlimit-mask 32 -j REJECT --reject-with tcp-reset
    iptables -A GAME_PROTECTION -p udp --dport "$SEVEN_DAYS_PORT" -m hashlimit --hashlimit-above 50/sec --hashlimit-burst 100 --hashlimit-mode srcip --hashlimit-name sevendays_udp -j DROP
...
if [ -n "$CUSTOM_TCP_PORTS" ]; then
    for port in $CUSTOM_TCP_PORTS; do
        ...
        iptables -A GAME_PROTECTION -p tcp --syn --dport "$port" -m connlimit --connlimit-above "$CUSTOM_TCP_CONNLIMIT" --connlimit-mask 32 -j REJECT --reject-with tcp-reset
        TCP_PORTS+=("$port")
    done
fi
...
if [ -n "$CUSTOM_UDP_PORTS" ]; then
    for port in $CUSTOM_UDP_PORTS; do
        ...
        iptables -A GAME_PROTECTION -p udp --dport "$port" -m hashlimit --hashlimit-above "$CUSTOM_UDP_LIMIT/sec" --hashlimit-burst "$((CUSTOM_UDP_LIMIT*2))" --hashlimit-mode srcip --hashlimit-name custom_udp_$port -j DROP
    done
fi
...
if [ "$GENERAL_TCP_SYN_ENABLE" == "true" ] && [ ${#TCP_PORTS[@]} -gt 0 ]; then
    ...
    # Using 'recent' module to block aggressive IPs sending too many SYNs
    iptables -A GAME_PROTECTION -p tcp --syn -m multiport --dports "$COMMA_PORTS" -m recent --name synflood --set
    iptables -A GAME_PROTECTION -p tcp --syn -m multiport --dports "$COMMA_PORTS" -m recent --name synflood --update --seconds 1 --hitcount "$GENERAL_TCP_SYN_LIMIT" -j DROP
fi
